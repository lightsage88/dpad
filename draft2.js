...
//ajax requests are present in 
//selectGame
//useToken
//researchQueryGames
//gatherInfoOnOtherGames

function ajaxTime(searchTerm) {
	$.when(
		$.ajax({
			type:'get',
			url: 'https://www.giantbomb.com/api/search',
			data: {api_key: `${gBApiKey}`,
					query: `${searchTerm}`,
					resource: 'games',
					format: 'jsonp',
					done: function (data) {
						console.log(data);
						let array1 = jQuery.makeArray(data)
						let gameCode = array1[0].api_detail_url.slice(35).replace('/','');
							//consider returning this globally
							}
					},
			dataType: 'jsonp'
		}),
		$.ajax({
			type: 'get',
			url: `https://www.giantbomb.com/api/game/${gameCode}/`,
			data: {
				api_key: gBApiKey,
				format: 'jsonp',
				json_callback: function(data) {
						let devCode = data.developers[0].api_detail_url.slice(38).replace('/','');
					},
				field_list: 'name, original_release_date, image, developers, platforms, reviews'
			},
			dataType: 'jsonp'
		}),
		$.ajax({
...
// }
// //data.results[0].api_detail_url.slice(35).replace('/','');
...
// 			} else if(gameLibrary[i] > gameLibrary[i++].year) {
// 				gameLibrary[i] = gameLibrary[i--];
// 			} else {
...
// //developers[0].api_detail_url.slice(38).replace('/','');
